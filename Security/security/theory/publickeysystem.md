...
{% stepper %}
{% step %}
데이터를 암호화할 때 공용키(public key)를 사용한다.
{% endstep %}

{% step %}
암호화된 데이터는 오직 개인키(private key)로만 복호화할 수 있다.
{% endstep %}

{% step %}
개인키는 절대 노출되지 않으며, 이를 통해 보안성을 높인다.
{% endstep %}
{% endstepper %}
...
| 알고리즘           | 특징                                  |
| -------------- | ----------------------------------- |
| RSA            | 가장 널리 사용되는 비대칭 암호화 알고리즘             |
| ECC            | RSA보다 짧은 키 길이로 같은 수준의 보안을 제공하는 알고리즘 |
| DSA            | 전자서명에 최적화된 알고리즘                     |
| Diffie-Hellman | 보안 키 교환(Key Exchange) 알고리즘          |
